    ...
    for i in reversed(range(index)):
        if (ld_sd_queue[i].op == 'Sd'):
            if (ld_sd_queue[i].address == ld_sd_queue[index].address):
                if (type(ld_sd_queue[i].data) == int) or (type(ld_sd_queue[i].data) == float):
                    data.append(ld_sd_queue[i].data)
            break
    return data

# Check if previous all Sds are ready and un-matched
...
def mem(ld_sd_queue, ld_sd_mem, time_ld_sd_mem, results_buffer,
    ...
    for element in ld_sd_queue:
        index += 1
        if (index >= 1) and (element.op == 'Ld') and (element.ready == 1):
    ...
    if ld_sd_mem.busy == 1:
        ...
        if ld_sd_mem.cycle == time_ld_sd_mem:
            ...
            # put data into memory for Sd
                ...
                memory[int(ld_sd_mem.address)] = ld_sd_mem.data

    # fetch new instruction into ld_sd_mem
    if (ld_sd_mem.busy == 0) and (len(ld_sd_queue) > 0):
        # flag_1st_ldsd
        ...
        if (ld_sd_queue[0].op == 'Sd'):
            ...
            if (ld_sd_queue[0].ready == 1):
                ...
                if (check_if_sd_committable(ld_sd_queue, ROB)):
                    ...
                    if (type(ld_sd_queue[0].data) == int) or (type(ld_sd_queue[0].data) == float):
        ...
        if (not flag_1st_ldsd_sent):
            for index in range(len(ld_sd_queue)):
                # ready Ld
                if (ld_sd_queue[index].op == 'Ld') and (ld_sd_queue[index].ready == 1):
                    # haven't got data
                    if (not type(ld_sd_queue[0].data) == int) and (not type(ld_sd_queue[0].data) == float):
